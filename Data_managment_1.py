...
class LLMInitializer:
    @staticmethod
    def initialize_llm(model_path="/home/nandha/llama_models/llama-2-13b-chat.Q4_K_M.gguf"):
        """Initialize the GGUF model with optimized settings"""
        try:
            log(f"Initializing GGUF model from: {model_path}")
            # Initialize Llama model with optimal parameters for 13B model
            llm = Llama(
...
class LLMEmailAnalyzer:
    ...
    def setup_prompt_templates(self):
        # Modified templates for Llama-2 chat format
        self.templates = {
            'summary': """<s>[INST]Analyze this email content and provide:
                ...
                Email content:
                {content}[/INST]""",
            'sentiment': """<s>[INST]Analyze the sentiment and urgency:
                ...
                Content:
                {content}[/INST]""",
            'technical': """<s>[INST]Analyze technical aspects and provide:
...
def main():
    try:
        # Initialize LLM with local GGUF model
        log("Starting email analysis process...")
        
        # Update this path to your local GGUF model location
        model_path = "C:/path/to/your/llama-2-13b-chat.Q4_K_M.gguf"
        
        llm = LLMInitializer.initialize_llm(model_path)
